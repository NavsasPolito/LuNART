function msg = PlotDopplerShiftDiff(directory, file1, file2, colors, limit, epochDate, varargin)
...
for i = 1:length(m1.obsSolutions)
    ...
    if ~isempty(j)
        ...
        plot(time, m1.obsSolutions(i).Doppler(timeIdx) - m2.obsSolutions(j).Doppler(timeIdx) ,'Color',colors(k).Color,'DisplayName',svID,'LineWidth',1.1);
    else
...
title('Doppler Shift difference (RAW minus REF)');
Pix_SS = get(0,'screensize');
h.OuterPosition = [Pix_SS(3)/2 Pix_SS(2) Pix_SS(3)/2 Pix_SS(4)/3];

%--- Message
msg = append(msg,"Doppler shift difference plot generated");